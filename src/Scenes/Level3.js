class Level3 extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.image("goldduck", "duck_yellow.png");
    }
    ...
    create() {
        ...
        this.goldcurve = new Phaser.Curves.Spline([100, 300, 900, 300, 100, 300]);
        this.goldduck = this.add.follower(this.goldcurve, 100, 300, "goldduck");
        this.goldduck.setScale(0.65);
        this.goldduck.startFollow({
            from: 0,
            to: 1,
            delay: 0,
            duration: 2000,
    ...
    update() {
        ...
        for(let i = 0; i<5; i++){
            if(typeof(this.duckbullets[i]) == "object"){
                ...
                else if(this.duckbullets[i].y > 700 && Math.abs(this.duckbullets[i].x - (this.my.sprite.player.x-20)) < 40){
                    this.duckbullets[i].visible = false;
                    this.duckbullets[i] = undefined;
                    this.bulletCount -=1;
                    this.sound.play("hit");
        ...
        for(let i = 0; i<10; i++){
            if(typeof(this.bullets[i]) == "object"){
                ...
                if(this.bullets[i].y < -20){
                    ...
                    for(let t = 0; t<6; t++){
                        if(typeof(this.targets[t]) == "object"){
                            if(Math.abs((this.targets[t].x - this.bullets[i].x)) < 60 && Math.abs((this.targets[t].y - this.bullets[i].y)) < 50){
                                ...
                                if(typeof(this.goldduck) == "object"){
                                    roundOver = false;
                                }
                                if(roundOver){
                                    this.registry.set('score', this.score);
                    ...
                    if(typeof(this.bullets[i]) == "object" && typeof(this.goldduck) == "object" && Math.abs((this.goldduck.x - this.bullets[i].x)) < 40 && Math.abs((this.goldduck.y - this.bullets[i].y)) < 20){
                        this.goldduck.visible = false;
                        this.goldduck = undefined;
                        this.bullets[i].visible = false;
                        this.bullets[i] = undefined;
                        this.sound.play("hit");
                        this.score += 7;
